"""
Simple working HTML visualization - No template literal issues
"""
...
def create_working_html(kg: KnowledgeGraphSource, drug_name: str, max_neighbors: int = 30):
    """Create a working D3.js visualization"""
    ...
    # Create HTML with proper string concatenation (no template literals)
    html = '''<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
    ...
    <p>Showing ''' + str(len(neighbors)) + ''' connections | Drag nodes to explore | Click for details</p>
    ...
    <script>
        ...
        function clicked(event, d) {
            ...
            let infoHTML = '<h3>' + d.name + '</h3>';
            infoHTML += '<p><strong>Type:</strong> ' + d.type + '</p>';
            infoHTML += '<p><strong>ID:</strong> ' + d.id + '</p>';
            infoHTML += '<p><strong>Connections:</strong> ' + connections.length + '</p>';
            infoHTML += '<h4>Relationships:</h4><ul>';

            connections.slice(0, 5).forEach(c => {
                const other = c.source.id === d.id ? c.target : c.source;
                infoHTML += '<li>' + c.relation + ': ' + other.name + '</li>';
            });

            if (connections.length > 5) {
                infoHTML += '<li>... and ' + (connections.length - 5) + ' more</li>';
            }
            infoHTML += '</ul>';
    ...
    # Save file
    filename = f"visualizations/{drug_name.replace(' ', '_')}.html"
    ...
    print(f"   2. Press Cmd+O (or File > Open)")
    print(f"   3. Navigate to and open: {abs_path}")
    print(f"\n   Or run: open \"{filename}\"")


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python create_viz.py <drug_name>")
        print("Example: python create_viz.py metformin")
        sys.exit(1)
    ...
    create_working_html(kg, drug_name)
